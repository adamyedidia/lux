...
wall_img = blurDnClr(double(imread(wall_cal)), nlevs, filter);
[iold, jold, ii, jj, ~] = rectify_image_solve(wall_img(:,:,1));
wall_img = rectify_image(wall_img, iold, jold, ii, jj);

% get the ceiling corners and door corners again
figure; imagesc(wall_img(:,:,1)); axis equal;
title('choose ceiling corner and floor corner');
ceil_corners = ginput(2);
hold on; plot(ceil_corners(:,1), ceil_corners(:,2), 'ro');
wall_height = sqrt((ceil_corners(1,1) - ceil_corners(2,1))^2 + ...
    (ceil_corners(1,2) - ceil_corners(2,2))^2);

title('choose door corners from top left, clockwise');
door_corners = ginput(4);
hold on; plot(door_corners(:,1), door_corners(:,2), 'bo');
width1 = sqrt((door_corners(1,1) - door_corners(2,1))^2 + ...
    (door_corners(1,2) - door_corners(2,2))^2);
width2 = sqrt((door1(1) - door2(1))^2 + (door1(2) - door2(2))^2);
door_height = sqrt((door_corners(2,1) - door_corners(3,1))^2 + ...
    (door_corners(2,2) - door_corners(3,2))^2);
