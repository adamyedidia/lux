function [iold, jold, ii, jj, rectified] = rectify_image_solve(caliImg, refdims, verbose) 
...
if nargin < 2
    ...
    height = refdims(1);
    width = refdims(2);
    pts_ref = [0 0; 0 width; height width; height 0];
end

[h, w, ~] = size(caliImg);
sz = min(h,w)/3; 

figure; imagesc(caliImg(:,:,2));
...
[rectified] = rectify_image(caliImg, iold, jold, ii, jj);

if verbose
    figure;imagesc(rectified./255, 'XData',wvec,'YData',hvec); axis equal; 
