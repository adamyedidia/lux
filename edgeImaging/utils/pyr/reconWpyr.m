...
if any(levs == 1)
  if (res_sz(1) == 1)
    res = upConv(pyrBand(pyr,ind,1), hfilt', edges, [1 2], [1 2], res_sz, res);
  elseif (res_sz(2) == 1)
    res = upConv(pyrBand(pyr,ind,1), hfilt, edges, [2 1], [2 1], res_sz, res);
  else
    if any(bands == 1) % horizontal
      ires = upConv(pyrBand(pyr,ind,1),filt',edges,[1 2],[1 stag],hres_sz);
      res = upConv(ires,hfilt,edges,[2 1],[2 1],res_sz,res);  %destructively modify res
      %% billf feb. 2005 added assignment to res, since upconv doesn't
      %% behave as desired with destructive modification.
    end
    if any(bands == 2) % vertical
      ires = upConv(pyrBand(pyr,ind,2),hfilt',edges,[1 2],[1 2],lres_sz);
      res = upConv(ires,filt,edges,[2 1],[stag 1],res_sz,res);  %destructively modify res
        %% billf feb. 2005 added assignment to res, since upconv doesn't
      %% behave as desired with destructive modification.
    end
