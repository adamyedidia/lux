...
def getErrorMakerTKnownTargetReflectanceUnknown(d, T, reflectance, observation):
    def getError(nuPhiReflectanceArray):
        reflectance = nuPhiReflectanceArray[2]
        nuPhiArray = np.array([nuPhiReflectanceArray[0], nuPhiReflectanceArray[1]])

        modelWoReflectance = modelMakerTKnown(d, T)(nuPhiArray)

        model = [reflectance*i for i in modelWoReflectance]
        return -computeLogLikelihood(observation, model)
...
def getObservationTwoPointPlaneTKnownReflectanceUnknown(d, T, nuPhiArray, reflectance):
    model = modelMakerTKnown(d, T)(nuPhiArray)

    observation = poissonify([i*reflectance for i in model])
...
def exhaustiveSearchHack2(errorFunc, minNu, minPhi, minRefl, maxNu, maxPhi, maxRefl, observation):
...
#            nuPhiArray = np.array([nu, phi])
            ...
            while refl < maxRefl:
                ...
                nuPhiReflectanceArray = np.array([nu, phi, refl])

                print((nu, phi, refl))

                error = errorFunc(nuPhiReflectanceArray)

                if error < bestError:
                    bestError = error
                    bestMinNuArray = nuPhiReflectanceArray
...
observation = getObservationTwoPointPlaneTKnownReflectanceUnknown(d, T, np.array([nu, phi]), reflectance)

#print sum(observation)

#jack3d.plotArray(model)
#jack3d.plotArray(addBackgroundNoise(model))
#jack3d.plotArray(observation)
...
errorFunc = getErrorMakerTKnownTargetReflectanceUnknown(d, T, reflectance, observation)
...
p.xlabel("Time")
p.ylabel("Intensity")
p.savefig("model_red.png")

#gradientDescent(errorFunc, np.array([pi/2, pi/2]), 1e-8, 1e-5, modelFunc, observation)

print(exhaustiveSearchHack2(errorFunc, 0, 0, SEARCH_EPS, pi/2, pi/2, 1, observation))
